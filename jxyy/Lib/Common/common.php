...
function UU($model,$params,$redirect=false,$suffix=false){
	...
	if(C('url_rewrite')){
		...
		if($model == 'Home-vod/show'){
			$reurl = str_replace(array('$id','$page','$wd','$area','$language','$actor','$director','$year','$letter','$order'),array($params['id'],$params['p'],$params['wd'],$params['area'],$params['language'],$params['actor'],$params['director'],$params['year'],$params['letter'],$params['order']),C('rewrite_vodlist'));
			...
			$reurl = str_replace(array('$wd','$page','$area','$language','$actor','$director','$year','$letter','$order'),array($params['wd'],$params['p'],$params['area'],$params['language'],$params['actor'],$params['director'],$params['year'],$params['letter'],$params['order']),C('rewrite_vodsearch'));
...
// Tag链接
function ff_tag_url($str,$sid=''){
	if($sid==2){
		return UU('Home-tag/shown',array('wd'=>urlencode($str)),false,true);
	}else{
		return UU('Home-tag/show',array('wd'=>urlencode($str)),false,true);
	}
}
// 内容页Tag链接
...
function ff_order_by($order = 'addtime'){
	...
	$arrorder['down'] = 'down';
	$arrorder['gold'] = 'gold';
	$arrorder['golder'] = 'golder';
	$arrorder['year'] = 'year';
	$arrorder['letter'] = 'letter';
